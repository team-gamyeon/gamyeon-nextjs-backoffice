...
const INITIAL_NOTICES: Notice[] = [
  ...
  },
  {
    ...
    content:
      "2025년 12월 31일 오후 11시~자정 연말 서버 점검이 진행되었습니다.",
    isActive: false,
    ...
    >
      ...
      <div className="flex items-center gap-4 text-sm">
        <span className="text-muted-foreground">
          전체{" "}
          <span className="font-semibold text-foreground">{notices.length}</span>
          개
        </span>
        <span className="text-muted-foreground">
          활성{" "}
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <Button size="sm" className="h-9 gap-1.5 cursor-pointer" onClick={handleAdd}>
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-3 px-4 py-3">
                ...
                {/* 질문 관리와 동일한 스타일의 토글 버튼 */}
                <button
                  type="button"
                  onClick={() => handleToggle(notice.id)}
                  className={cn(
                    "cursor-pointer inline-flex h-7 w-20 shrink-0 items-center justify-center rounded-full text-xs font-medium transition-colors",
                    notice.isActive
                ...
                <button
                  ...
                  className="cursor-pointer flex h-7 w-7 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-accent hover:text-foreground"
                >
